...
- `src/context/AppDataContext.tsx`'s mount-time `useEffect` wires `startSyncPull` in: each handler merges the incoming rows via `sync/merge.ts`, writes to `store.ts` + state, and `enqueue()`s any `findLocalWins` corrections. Reads "current" state via a `latestRef` (mirrored by a plain no-deps `useEffect`), **not** the functional `setState(current => ...)` form — the handlers need to call `enqueue()` as a side effect when local wins, and StrictMode double-invokes updater functions in dev specifically to catch impurities like that, which would double-push every correction.
