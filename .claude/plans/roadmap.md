...
- **Execution status (2026-09-16)**: Tasks 10 (Vitest + core-logic tests), 11 (Firestore adapter, device id, shared-PIN gate — verified end-to-end against the real `stall-8e1fc` project), 12 (outbox wired into every `persistX` helper), and 13 (draining the outbox — `src/sync/drain.ts`) are done. All Phase 2 work now lands as one commit per task on a single `phase2` branch (not a stack of per-task `feature/*` branches).
  ...
  - **Next**: Task 14 (pull + merge/conflict resolution).
